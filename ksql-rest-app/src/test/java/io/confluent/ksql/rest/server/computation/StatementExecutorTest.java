...
public class StatementExecutorTest extends EasyMockSupport {
  ...
  public void shouldBuildQueriesWithPersistedConfig() {
    ...
    expect(mockEngine.plan(eq(configuredCsas))).andReturn(plan);
    expect(mockEngine.execute(eqConfigured(plan)))
  ...
  private PersistentQueryMetadata mockReplayCSAS(
    ...
    expect(mockEngine.getPersistentQueries()).andReturn(ImmutableList.of());
    expect(mockEngine.plan(eqConfigured(csas)))
        .andReturn(mockPlan);
    expect(mockEngine.execute(eqConfigured(mockPlan)))
  ...
  public void shouldCascade4Dot1DropStreamCommand() {
    ...
    final KsqlPlan plan = Mockito.mock(KsqlPlan.class);
    expect(mockEngine.plan(eqConfigured(PreparedStatement.of("DROP", mockDropStream))))
        .andReturn(plan);
    expect(mockEngine.execute(eqConfigured(plan))).andReturn(ExecuteResult.of("SUCCESS"));
    replayAll();
  ...
  public void shouldNotCascadeDropStreamCommand() {
    ...
    final KsqlPlan plan = Mockito.mock(KsqlPlan.class);
    expect(mockEngine.plan(eqConfigured(statement))).andReturn(plan);
    expect(mockEngine.execute(eqConfigured(plan)))
